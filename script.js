(() => {
  ...
  function matchesQuery(product) {
    const aliases = { corredera: ["corredera", "corrediza"], corrediza: ["corrediza", "corredera"], ventana: ["ventana", "ventanas"], ventanas: ["ventana", "ventanas"], perfil: ["perfil", "perfiles", "marco"], perfiles: ["perfil", "perfiles", "marco"], tirador: ["tirador", "manilla"], asa: ["asa", "manilla"], manijas: ["manija", "manilla"], rieles: ["rieles", "riel"], cierre: ["cierre", "cerradero", "cremona"], negro: ["negro", "antracita"], cafe: ["cafe", "nogal"] };
  ...
  function bindEvents() {
    $("#search-form").addEventListener("submit", (event) => { event.preventDefault(); $("#search-suggestions").hidden = true; $("#catalogo").scrollIntoView({ behavior: "smooth" }); }); $("#search-input").addEventListener("input", (event) => { state.query = event.target.value; $("#search-clear").hidden = !state.query; renderSearchSuggestions(state.query); renderProducts(true); }); $("#search-input").addEventListener("focus", (event) => renderSearchSuggestions(event.target.value)); $("#search-clear").addEventListener("click", () => { state.query = ""; $("#search-input").value = ""; $("#search-clear").hidden = true; $("#search-suggestions").hidden = true; renderProducts(true); });
  ...
  renderFilterOptions(); initConfigurator(); initTransformation(); bindEvents(); closePanels(false); renderProducts(); renderCart(); renderRecent(); renderCompare(); $("#year").textContent = new Date().getFullYear();
})();
