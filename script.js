(() => {
  ...
  function openPanel(name) { closePanels(); $("#overlay").hidden = false; document.body.classList.add("is-locked"); const panel = name === "cart" ? $("#cart-drawer") : $("#filters"); panel.classList.add("is-open"); if (name === "cart") panel.setAttribute("aria-hidden", "false"); setTimeout(() => $(".icon-close", panel)?.focus(), 50); }
  function closePanels() { $("#overlay").hidden = true; document.body.classList.remove("is-locked"); $("#cart-drawer").classList.remove("is-open"); $("#cart-drawer").setAttribute("aria-hidden", "true"); $("#filters").classList.remove("is-open"); }
